...
import { getMonthlyComparisonAction } from '@/app/actions'; // Importe a action
...
export default function HomeTab({
  ...
  // Busca dados de comparação ao carregar
  useEffect(() => {
        ...
        <div className="bg-[#1f1630] p-6 rounded-3xl border border-white/5 shadow-lg relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-2">
              ...
              {/* BADGE DE COMPARAÇÃO VISUAL */}
              {comparison && (
                <div className={`text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1 border ${comparison.increased
                    ? 'bg-red-500/10 text-red-400 border-red-500/20'
                    : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                  }`}>
                  {comparison.increased ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                  {Math.abs(comparison.diffPercent)}% vs mês ant.
                </div>
              )}
            </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#1a1025', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }} itemStyle={{ color: '#fff' }} formatter={(val: any) => `R$ ${Number(val).toFixed(2)}`} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData}>
                ...
                <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ backgroundColor: '#1a1025', border: 'none', borderRadius: '8px' }} formatter={(val: any) => `R$ ${Number(val).toFixed(2)}`} />
